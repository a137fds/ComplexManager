...
const languagePreferencePrompt: Record<Language, string> = { en:'Use this language as your default language for the website and documents?', ru:'Использовать этот язык как язык по умолчанию для сайта и документов?', tr:'Bu dil web sitesi ve belgeler için varsayılan dil olarak kullanılsın mı?', fr:'Utiliser cette langue comme langue par défaut pour le site et les documents ?', da:'Skal dette være standardsproget for webstedet og dokumenter?', sv:'Ska detta vara standardspråk för webbplatsen och dokument?', pl:'Ustawić ten język jako domyślny dla strony i dokumentów?' };
...
export const App: React.FC = () => {
  ...
  useEffect(() => { if (profile?.default_language) setCurrentLang(profile.default_language as Language); }, [profile?.default_language]);
  const handleLanguageChange = async (lang: Language) => { setCurrentLang(lang); if (!profile?.id || profile.default_language === lang) return; if (!window.confirm(languagePreferencePrompt[lang])) return; const { error } = await supabase.from('user_profiles').update({ default_language: lang }).eq('id', profile.id); if (error) console.error('Failed to save default language:', error); };
  const refreshDatabaseData = async () => { setDbLoading(true); try { const [loadedComplexes, loadedBuildings] = await Promise.all([databaseApi.getComplexes(), databaseApi.getBuildings()]); setComplexes(loadedComplexes); setDbBuildings(loadedBuildings); setSelectedComplexId(current => current && loadedComplexes.some(c => c.ComplexID === current) ? current : loadedComplexes[0]?.ComplexID ?? null); } catch (error) { console.error('Failed to load database data:', error); } finally { setDbLoading(false); } };
  ...
  return <div className="min-h-screen bg-slate-100/70 text-slate-800 font-sans flex flex-col antialiased selection:bg-teal-600 selection:text-white"><Header complex={complex} currentRole={currentRole} currentLang={currentLang} onLangChange={handleLanguageChange} pendingAuditsCount={pendingAuditsCount}/><div className="flex-1 max-w-7xl w-full mx-auto flex flex-col md:flex-row"><Sidebar currentTab={currentTab} onTabSelect={setCurrentTab} currentRole={currentRole} currentLang={currentLang}/><main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto">
  ...
  </main></div>{activeInvoiceModal&&<InvoiceModal invoice={activeInvoiceModal} complex={complex} onClose={()=>setActiveInvoiceModal(null)} currentLang={currentLang}/>}</div>;
};