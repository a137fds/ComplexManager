...
const startLabels: Record<Language, { start: string; settings: string; role: string; roleDesc: string; law: string; governance: string }> = {
  en: { start: 'Start Page', settings: 'Administration', role: 'Role Permissions', roleDesc: 'Permissions are managed centrally in Administration by an administrator.', law: '634 Sayılı Kat Mülkiyeti Kanunu', governance: 'Complex Governance & Audit Standard' },
  ru: { start: 'Обзор', settings: 'Администрирование', role: 'Права ролей', roleDesc: 'Права доступа централизованно управляются администратором в разделе администрирования.', law: 'Закон Турции №634 о кондоминиумах', governance: 'Управление комплексом и аудит' },
  tr: { start: 'Genel Bakış', settings: 'Yönetim', role: 'Rol Yetkileri', roleDesc: 'Yetkiler yönetici tarafından Yönetim bölümünde merkezi olarak yönetilir.', law: '634 Sayılı Kat Mülkiyeti Kanunu', governance: 'Kompleks Yönetimi ve Denetim Standardı' },
  fr: { start: 'Vue d’ensemble', settings: 'Administration', role: 'Permissions des rôles', roleDesc: 'Les permissions sont gérées centralement dans l’administration par un administrateur.', law: 'Loi n° 634 sur la copropriété en Turquie', governance: 'Gouvernance et audit de la résidence' },
  da: { start: 'Oversigt', settings: 'Administration', role: 'Rollebaserede rettigheder', roleDesc: 'Rettigheder administreres centralt af en administrator under Administration.', law: 'Lov nr. 634 om ejerlejligheder i Tyrkiet', governance: 'Kompleksstyring og revision' },
  sv: { start: 'Översikt', settings: 'Administration', role: 'Rollbehörigheter', roleDesc: 'Behörigheter hanteras centralt av en administratör under Administration.', law: 'Turkiets lag nr 634 om ägarlägenheter', governance: 'Komplexstyrning och revision' },
  pl: { start: 'Przegląd', settings: 'Administracja', role: 'Uprawnienia ról', roleDesc: 'Uprawnienia są centralnie zarządzane przez administratora w Administracji.', law: 'Turecka ustawa nr 634 o własności lokali', governance: 'Zarządzanie kompleksem i audyt' }
};

export const Sidebar: React.FC<SidebarProps> = ({ currentTab, onTabSelect, currentLang }) => {
  const { hasPermission } = useAuth();
  const t = translations[currentLang];
  const s = startLabels[currentLang];
  const navItems = [
    ...
    { id: 'residents' as TabType, label: t.navResidents, icon: Users, visible: hasPermission('residents.view') },
