...
import { Building2, Layers, Users, CreditCard, Wrench, FileText, Settings, Info, Home } from 'lucide-react';
import { UserRole, Language } from '../types';
import { translations } from '../i18n/translations';
import { useAuth } from '../auth/AuthContext';

export type TabType = 'start_page' | 'complex' | 'buildings' | 'residents' | 'billing' | 'tasks' | 'documents' | 'administration' | 'guest_overview';
interface SidebarProps { currentTab: TabType; onTabSelect: (tab: TabType) => void; currentRole: UserRole; currentLang: Language; }
const startLabels: Record<Language, { start: string; settings: string; role: string; roleDesc: string; law: string; governance: string; apartments: string }> = {
  en: { start: 'Start Page', settings: 'Administration', role: 'Role Permissions', roleDesc: 'Permissions are managed centrally in Administration by an administrator.', law: '634 Sayılı Kat Mülkiyeti Kanunu', governance: 'Complex Governance & Audit Standard', apartments: 'Apartments' },
  ru: { start: 'Обзор', settings: 'Администрирование', role: 'Права ролей', roleDesc: 'Права доступа централизованно управляются администратором в разделе администрирования.', law: 'Закон Турции №634 о кондоминиумах', governance: 'Управление комплексом и аудит', apartments: 'Квартиры' },
  tr: { start: 'Genel Bakış', settings: 'Yönetim', role: 'Rol Yetkileri', roleDesc: 'Yetkiler yönetici tarafından Yönetim bölümünde merkezi olarak yönetilir.', law: '634 Sayılı Kat Mülkiyeti Kanunu', governance: 'Kompleks Yönetimi ve Denetim Standardı', apartments: 'Daireler' },
  fr: { start: 'Vue d’ensemble', settings: 'Administration', role: 'Permissions des rôles', roleDesc: 'Les permissions sont gérées centralement dans l’administration par un administrateur.', law: 'Loi n° 634 sur la copropriété en Turquie', governance: 'Gouvernance et audit de la résidence', apartments: 'Appartements' },
  da: { start: 'Oversigt', settings: 'Administration', role: 'Rollebaserede rettigheder', roleDesc: 'Rettigheder administreres centralt af en administrator under Administration.', law: 'Lov nr. 634 om ejerlejligheder i Tyrkiet', governance: 'Kompleksstyring og revision', apartments: 'Lejligheder' },
  sv: { start: 'Översikt', settings: 'Administration', role: 'Rollbehörigheter', roleDesc: 'Behörigheter hanteras centralt av en administratör under Administration.', law: 'Turkiets lag nr 634 om ägarlägenheter', governance: 'Komplexstyrning och revision', apartments: 'Lägenheter' },
  pl: { start: 'Przegląd', settings: 'Administracja', role: 'Uprawnienia ról', roleDesc: 'Uprawnienia są centralnie zarządzane przez administratora w Administracji.', law: 'Turecka ustawa nr 634 o własności lokali', governance: 'Zarządzanie kompleksem i audyt', apartments: 'Mieszkania' }
};
