...
export const ResidentsView: React.FC<ResidentsViewProps> = ({ currentLang, currentRole }) => {
  ...
  return <div className="space-y-10"><ApartmentsView apartments={apartments} buildings={buildings} currentRole={currentRole} currentLang={currentLang} onRefresh={async () => { await load(); }} /><OwnersView currentRole={currentRole} currentLang={currentLang} /></div>;
};
