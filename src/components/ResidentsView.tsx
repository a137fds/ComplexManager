...
export const ResidentsView: React.FC<ResidentsViewProps> = ({ currentLang, currentRole }) => {
  ...
  const load = async () => {
    ...
    setBuildings(buildingRows.map((b: any) => ({
      id: String(b.id), blockCode: b.name, name: b.name, totalFloors: 0, totalUnits: 0,
      occupiedUnits: 0, caretakerName: '', caretakerPhone: '', elevatorCount: 0,
      heatingType: '', photos: [], documentsCount: 0, notes: ''
    })));

    setApartments((apartmentsResult.data || []).map((a: any) => {
      const owners = ownerRows.filter((o: any) => Number(o.apartment_id) === Number(a.id)).map((o: any): ApartmentOwner => {
        const u: any = userMap.get(o.user_id);
        return { userId: o.user_id, firstName: u?.first_name || '', lastName: u?.last_name || '', email: u?.email || '', defaultLanguage: u?.default_language || 'en' };
      });
      return {
        id: Number(a.id), buildingId: Number(a.building_id), buildingName: buildingMap.get(Number(a.building_id)) || '',
        apartmentNumber: a.apartment_number, owners,
        invoiceRecipientIds: recipientRows.filter((r: any) => Number(r.apartment_id) === Number(a.id)).map((r: any) => r.user_id)
      };
    }));
  };
